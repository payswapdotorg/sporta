/**
 * Source-scan pins (the W602/W603/W703/W706/W604/W306 precedent): the two
 ...
 *    the declared runtime dependencies (exactly `zod` — the
 *    @sporta/contracts precedent; the real pipeline packages named by the
 *    policy table are DEV dependencies, consumed only by the consistency
 *    tests) or a local relative module. No undeclared package can enter the
 *    runtime dependency set silently — W805 can depend on this package
 *    without dragging the benchmark graph.
 * 2. **Constitution**: zero `Math.random(…)`, `Date.now(…)`,
 *    `performance.now(…)`, and `new Date(…)` CALLS in `src` AND `scripts`
 *    (both are shipped package surface) — evaluation is a pure function
 *    over one window; wall time is never read, randomness never used.
 *
 ...
 * exactly zod (the repo's one allowed external runtime validation
 * dependency, per the @sporta/contracts precedent).
 */
  ...
  test("no Math.random(…), Date.now(…), performance.now(…), or new Date(…) call appears in any shipped module", () => {
    ...
    for (const modulePath of shipped) {
      ...
      for (const pattern of FORBIDDEN) {
        expect(
          pattern.test(source),
          `${modulePath} matches ${String(pattern)} — SLO evaluation is a pure function over one window (the constitution); wall time and randomness are never read`,
        ).toBe(false);
      }
    }
  });
