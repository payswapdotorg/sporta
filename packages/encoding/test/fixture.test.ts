...
describe("FixtureFrameEncoder (the deterministic fixture tier)", () => {
  ...
  test("identical inputs produce byte-identical outputs (deterministic)", () => {
    ...
    const a = encoder.encode({ source: { kind: "rgb24-frames", frames, width: FRAME_WIDTH, height: FRAME_HEIGHT }, fps: FRAME_FPS, origin });
    const b = encoder.encode({ source: { kind: "rgb24-frames", frames, width: FRAME_WIDTH, height: FRAME_HEIGHT }, fps: FRAME_FPS, origin });
  ...
  test("different frame content produces different bytes (the stream hash is in the document)", () => {
    const origin = { rendererId: "test", rendererVersion: "0.0.0", bridge: "raw-frames" as const };
    const a = encoder.encode({ source: { kind: "rgb24-frames", frames: syntheticFrames(6), width: FRAME_WIDTH, height: FRAME_HEIGHT }, fps: FRAME_FPS, origin });
    const different = syntheticFrames(6).map((frame, i) => {
      const copy = new Uint8Array(frame);
      copy[0] = (copy[0]! + 1 + i) % 256;
      return copy;
    });
    const b = encoder.encode({ source: { kind: "rgb24-frames", frames: different, width: FRAME_WIDTH, height: FRAME_HEIGHT }, fps: FRAME_FPS, origin });
    expect(a.contentHash).not.toBe(b.contentHash);
  });

  test("the bytes are a LABELED fixture document, never video", () => {
    const result = encoder.encode({
      source: { kind: "rgb24-frames", frames: syntheticFrames(3), width: FRAME_WIDTH, height: FRAME_HEIGHT },
      fps: FRAME_FPS,
  ...
  test("admission is fail-closed (bad geometry / bad fps / empty frames)", () => {
    const origin = { rendererId: "test", rendererVersion: "0.0.0", bridge: "raw-frames" as const };
    expect(() =>
      encoder.encode({ source: { kind: "rgb24-frames", frames: [new Uint8Array(8 * 8 * 3)], width: 8, height: 8 }, fps: 25, origin }),
    ).toThrow(EncodingError);
    expect(() =>
      encoder.encode({ source: { kind: "rgb24-frames", frames: [], width: FRAME_WIDTH, height: FRAME_HEIGHT }, fps: 25, origin }),
    ).toThrow(EncodingError);
    expect(() =>
      encoder.encode({ source: { kind: "rgb24-frames", frames: syntheticFrames(2), width: FRAME_WIDTH, height: FRAME_HEIGHT }, fps: -1, origin }),
    ).toThrow(EncodingError);
  });
  ...
  test("the staged-file tier derives its identity without reading the file (zero I/O)", () => {
    const origin = { rendererId: "test", rendererVersion: "0.0.0", bridge: "raw-frames" as const };
    const result = encoder.encode({
      source: { kind: "rgb24-file", path: "/nonexistent/staged.rgb24", frameCount: 4, width: FRAME_WIDTH, height: FRAME_HEIGHT },
      fps: FRAME_FPS,
      origin,
    });
