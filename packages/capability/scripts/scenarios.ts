/**
 * The eight canonical W901 fixture scenarios (SHARED MODULE).
 *
 * Both the generator (`scripts/regen-fixtures.ts`) and the freshness pin
 * (`test/fixtures.test.ts`) import this module, so the checked-in fixtures
 * can be proven "exactly as fresh as a live build" — the regen-golden
 * precedent (`packages/evaluation/scripts/regen-golden.ts`): the fixture's
 * canonical bytes must equal what `buildCapabilityResponse` produces for
 * these inputs RIGHT NOW, no matter how the file is whitespace-formatted.
 *
 * The scenarios are the brief's deny/degraded coverage list: anonymous,
 * authenticated-no-roles, rights-denied renderer, quota-exhausted,
...
/**
 * The eight canonical scenarios (inputs), in the brief's coverage-list order.
 * Each entry is a `buildCapabilityResponse` input.
 */
export const FIXTURE_SCENARIOS: Readonly<Record<string, unknown>> = {
...
/** The fixture file names, in the canonical scenario order. */
export const FIXTURE_NAMES: readonly string[] = Object.keys(FIXTURE_SCENARIOS);
