/**
 * Internal helpers of the presentation director (the camera-director
 * `internal.ts` convention — deliberately NOT exported from the package
 * barrel). Everything here is total over JSON-safe documents: no throws,
 * no clock, no RNG.
 */
 ...
 * consumers can never mutate the policy constants — the camera-director
 * posture).
 */
